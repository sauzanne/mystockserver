...
public class AmfController {
	...
	@Path("getAllAmfAlert")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getAllMeasureAlert(@QueryParam(PARAM_LOGIN) String login, @Context SecurityContext sc) {
		...
		try {
			return Response.ok(amfService.getAllMeasure(login), MediaType.APPLICATION_JSON).build();
		} catch (FunctionalException e) {
